...
1. **改代码 / 审代码 / 问答咨询都走 Fable 5 子代理**（model 已钉死为 `claude-fable-5[effort=xhigh]`，即 Fable 5 thinking max）：
   ...
   - 问答 / 咨询 / 方案建议 / 如何部署 → `fable5-advisor`（Fable 5 顾问）；实质性技术问答父代理也不得自答，必须委派
...
Key points (EN): all code writing/review AND substantive Q&A/consulting are delegated to Fable 5 thinking max subagents pinned via `model: claude-fable-5[effort=xhigh]`; every completed task must end with the executor sign-off line; rules live in `.cursor/rules/*.mdc`, agents in `.cursor/agents/*.md`, all repo-relative and portable.
